...
Color(11,3);
WriteLn('????????????????????????????????????????');
WriteLn('? Songname:                            ?'); For Ak:=4 to 16 Do
WriteLn('?                                      ?');
WriteLn('????????????????????????????????????????');
GotoXY(15,3); Write(Song);
For Ak:=4 to 16 Do
    Begin
...
For Ak:=1 to 5 Do Begin GotoXY(1,18+Ak);
              Write('?                                                                 ?');
GotoXY(1,24); Write('???????????????????????????????????????????????????????????????????'); End;
Color(14,2);
GotoXY(41,2); Write('??????????????????????');
GotoXY(41,3); Write('? Length:        + - ?');
GotoXY(41,4); Write('? Position:      + - ?');
GotoXY(41,5); Write('? Pattern:       + - ?');
GotoXY(41,6); Write('? Restart:       + - ?');
GotoXY(41,7); Write('? Octave:        + - ?');
GotoXY(41,8); Write('? Track:             ?');
GotoXY(41,9); Write('? Channel:           ?');
GotoXY(41,10); Write('??????????????????????'); Color(12,4);
GotoXY(41,11); Write('????????????????????????????????????????');
GotoXY(41,12); Write('? Play ? Pattern ? Clear Pattern       ?');
GotoXY(41,13); Write('????????????????????????????????????????');
GotoXY(41,14); Write('? Clear all ? Save ? Load ? Instrument ?');
GotoXY(41,15); Write('????????????????????????????????????????');
GotoXY(41,16); Write('? DOS Shell ? Print ? Exit             ?');
GotoXY(41,17); Write('????????????????????????????????????????'); Color(11,3);
Color(14,5);
GotoXY(68,18);    Write('?????????????'); For Ak:=1 to 5 Do Begin
GotoXY(68,Ak+18); Write('?           ?'); End;
GotoXY(68,24);    Write('?????????????');
Sm;
Pattern;
End;
...
WriteLn(Bt,'BRO-Tracker 2.0? - (C) JBC Soft, ',JBN,', 1995, All rights reserved');
Close(Bt);
Message('');
End;
...
While X<>#24 Do
      ...
      If (Mx>40) And (Mx<52) And (My=15) And (Ml) Then
         ...
         Color(15,4);
         GotoXY(1,1); Write('':80); GotoXY(1,1);
         WriteLn('BRO-Tracker 2.0(open) - (C) JBC Soft, ',JBN,', 1995, General Public License 3');
         InitMuis;
         Teken;
